#ifndef __FACETHINK_API_CLS_IMAGE_ROTATE_HPP__
#define __FACETHINK_API_CLS_IMAGE_ROTATE_HPP__
...
namespace facethink {
class EXPORT_CLASS ClsImageRotate {
 public:
   ...
   */
  EXPORT_CLASS_API static ClsImageRotate* create(const std::string& model_path, const std::string& config_path);
  ...
  EXPORT_CLASS_API virtual ~ClsImageRotate(void);

 protected:
  EXPORT_CLASS_API explicit ClsImageRotate(void);

 private:
  ClsImageRotate(const ClsImageRotate&);
  ClsImageRotate& operator=(const ClsImageRotate&);
};

}  // namespace facethink

#endif  //__FACETHINK_API_CLS_IMAGE_ROTATE_HPP__